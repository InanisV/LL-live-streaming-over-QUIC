    ...
    this.startInterval = function () {
        ...
        this.timer = setInterval(function () {
            ...
            if (self.player && self.player.isReady()) {
                ...
                var currentLatency = parseFloat(
                    self.player.getCurrentLiveLatency(),
                    10
                );
                self.domElements.metrics.latencyTag.innerHTML =
                    currentLatency + " secs";

                var currentPlaybackRate = self.player.getPlaybackRate();
                self.domElements.metrics.playbackrateTag.innerHTML =
                    Math.round(currentPlaybackRate * 1000) / 1000;

                var currentBuffer = dashMetrics.getCurrentBufferLevel("video");
                self.domElements.metrics.bufferTag.innerHTML =
                    currentBuffer + " secs";
                ...
                self.domElements.metrics.min.innerHTML =
                    (minutes < 10 ? "0" : "") + minutes + ":";
                // 发布通知
                pubsub.publish("copy", METRIC_INTERVAL);
            }
        }, METRIC_INTERVAL);
    };
};
...
App.prototype._setDomElements = function () {
    this.domElements.settings.targetLatency =
        document.getElementById("target-latency");
    this.domElements.settings.maxDrift = document.getElementById("max-drift");
    this.domElements.settings.maxCatchupPlaybackRate = document.getElementById(
        "max-catchup-playback-rate"
    );
    this.domElements.settings.minCatchupPlaybackRate = document.getElementById(
        "min-catchup-playback-rate"
    );
    this.domElements.settings.catchupEnabled = document.getElementById(
        "live-catchup-enabled"
    );
    this.domElements.settings.abrThroughputRule =
        document.getElementById("abr-throughput");
    this.domElements.settings.abrBolaRule = document.getElementById("abr-bola");
    this.domElements.settings.abrInsufficientBufferRule =
        document.getElementById("abr-insufficient");
    this.domElements.settings.abrDroppedFramesRule =
        document.getElementById("abr-dropped");
    this.domElements.settings.abrAbandonRequestRule =
        document.getElementById("abr-abandon");
    this.domElements.settings.abrSwitchHistoryRule =
        document.getElementById("abr-switch");
    ...
    this.domElements.settings.targetLatency =
        document.getElementById("target-latency");
    this.domElements.settings.exportSettingsUrl = document.getElementById(
        "export-settings-url"
    );
    this.domElements.settings.additionalApisNetworkInformation =
        document.getElementById("network-api");
    ...
    this.domElements.chart.numberOfEntries = document.getElementById(
        "chart-number-of-entries"
    );

    this.domElements.metrics.latencyTag =
        document.getElementById("latency-tag");
    this.domElements.metrics.playbackrateTag =
        document.getElementById("playbackrate-tag");
    ...
    this.domElements.metrics.videoMaxIndex =
        document.getElementById("video-max-index");
    this.domElements.metrics.videoIndex =
        document.getElementById("video-index");
    this.domElements.metrics.videoBitrate =
        document.getElementById("video-bitrate");
};
            ...
            abr: {
                ...
                throughput: {
                    ...
                    lowLatencyDownloadTimeCalculationMode:
                        settings.downloadTimeEstimation,
...
App.prototype._adjustSettingsByUrlParameters = function () {
    ...
    if (params) {
        if (params.targetLatency !== undefined) {
            this.domElements.settings.targetLatency.value = parseFloat(
                params.targetLatency
            ).toFixed(1);
        }
        if (params.maxDrift !== undefined) {
            this.domElements.settings.maxDrift.value = parseFloat(
                params.maxDrift
            ).toFixed(1);
        }
        if (params.minCatchupPlaybackRate !== undefined) {
            this.domElements.settings.minCatchupPlaybackRate.value = parseFloat(
                params.minCatchupPlaybackRate
            ).toFixed(2);
        }
        if (params.maxCatchupPlaybackRate !== undefined) {
            this.domElements.settings.maxCatchupPlaybackRate.value = parseFloat(
                params.maxCatchupPlaybackRate
            ).toFixed(2);
        }
        if (params.abrThroughputRule !== undefined) {
            this.domElements.settings.abrThroughputRule.checked =
                params.abrThroughputRule === "true";
        }
        if (params.abrBolaRule !== undefined) {
            this.domElements.settings.abrBolaRule.checked =
                params.abrBolaRule === "true";
        }
        if (params.abrInsufficientBufferRule !== undefined) {
            this.domElements.settings.abrInsufficientBufferRule.checked =
                params.abrInsufficientBufferRule === "true";
        }
        if (params.abrAbandonRequestRule !== undefined) {
            this.domElements.settings.abrAbandonRequestRule.checked =
                params.abrAbandonRequestRule === "true";
        }
        if (params.abrSwitchHistoryRule !== undefined) {
            this.domElements.settings.abrSwitchHistoryRule.checked =
                params.abrSwitchHistoryRule === "true";
        }
        if (params.abrDroppedFramesRule !== undefined) {
            this.domElements.settings.abrDroppedFramesRule.checked =
                params.abrDroppedFramesRule === "true";
        }
        if (params.abrL2ARule !== undefined) {
            this.domElements.settings.abrL2ARule.checked =
                params.abrL2ARule === "true";
        }
        if (params.abrLoLPRule !== undefined) {
            this.domElements.settings.abrLoLPRule.checked =
                params.abrLoLPRule === "true";
        }
        if (params.catchupEnabled !== undefined) {
            this.domElements.settings.catchupEnabled.checked =
                params.catchupEnabled === "true";
        }
        ...
        if (params.additionalApisNetworkInformation !== undefined) {
            document.getElementById(
                params.additionalApisNetworkInformation
            ).checked = true;
        }
    }
};
...
App.prototype._getCurrentSettings = function () {
    var targetLatency = parseFloat(
        this.domElements.settings.targetLatency.value,
        10
    );
    var maxDrift = parseFloat(this.domElements.settings.maxDrift.value, 10);
    var minCatchupPlaybackRate = parseFloat(
        this.domElements.settings.minCatchupPlaybackRate.value,
        10
    );
    var maxCatchupPlaybackRate = parseFloat(
        this.domElements.settings.maxCatchupPlaybackRate.value,
        10
    );
    ...
    var abrInsufficientBufferRule =
        this.domElements.settings.abrInsufficientBufferRule.checked;
    var abrDroppedFramesRule =
        this.domElements.settings.abrDroppedFramesRule.checked;
    var abrAbandonRequestRule =
        this.domElements.settings.abrAbandonRequestRule.checked;
    var abrSwitchHistoryRule =
        this.domElements.settings.abrSwitchHistoryRule.checked;
    ...
    var additionalApisNetworkInformation =
        this.domElements.settings.additionalApisNetworkInformation.checked;
    var catchupMechanism = document.querySelector(
        'input[name="catchup"]:checked'
    ).value;
    var downloadTimeEstimation = document.querySelector(
        'input[name="download-estimation"]:checked'
    ).value;
    var throughputEstimation = document.querySelector(
        'input[name="throughput-estimation"]:checked'
    ).value;
...
App.prototype._setupLineChart = function () {
    ...
    this.chart = new Chart(this.domElements.chart.metricChart, config);
        ...
        if (self.player && self.player.isReady()) {
            ...
            if (data.datasets.length > 0) {
                ...
                if (self.chartData.lastTimeStamp) {
                    self.chartData.playbackTime +=
                        Date.now() - self.chartData.lastTimeStamp;
                }

                data.labels.push(
                    parseFloat(self.chartData.playbackTime / 1000).toFixed(3)
                );
                ...
                for (var i = 0; i < data.datasets.length; i++) {
                    if (
                        data.datasets[i].data.length > self.chartNumberOfEntries
                    ) {
                        data.datasets[i].data.shift();
                    }
                }
                data.datasets[0].data.push(
                    parseFloat(self.player.getCurrentLiveLatency()).toFixed(2)
                );
                ...
                data.datasets[2].data.push(
                    parseFloat(self.player.getPlaybackRate()).toFixed(2)
                );
...
App.prototype._adjustChartSettings = function () {
    if (!isNaN(parseInt(this.domElements.chart.interval.value))) {
        this.chartReportingInterval = parseInt(
            this.domElements.chart.interval.value
        );
    }

    if (!isNaN(parseInt(this.domElements.chart.numberOfEntries.value))) {
        this.chartNumberOfEntries = parseInt(
            this.domElements.chart.numberOfEntries.value
        );
    }
...
App.prototype._startIntervalHandler = function () {
    this.startInterval();
    // 暂停计时器
    pubsub.subscribe("pause", (data) => {
        clearInterval(this.timer);
    });
    // 恢复计时器
    pubsub.subscribe("reset", (data) => {
        this.startInterval();
    });
};
...
App.prototype._registerEventHandler = function () {
    ...
    document
        .getElementById("apply-settings-button")
    ...
    document
        .getElementById("chart-settings-button")
        .addEventListener("click", function () {
...
App.prototype._registerDashEventHandler = function () {
    this.player.on(
        dashjs.MediaPlayer.events.REPRESENTATION_SWITCH,
        this._onRepresentationSwitch,
        this
    );
};
...
App.prototype._onRepresentationSwitch = function (e) {
    try {
        if (e.mediaType === "video") {
            this.domElements.metrics.videoMaxIndex.innerHTML =
                e.numberOfRepresentations;
            this.domElements.metrics.videoIndex.innerHTML =
                e.currentRepresentation.index + 1;
